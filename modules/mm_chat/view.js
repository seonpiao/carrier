  ...
  var View = Base.extend({
    ...
    initChat: function() {
      ...
      if ($('.chat_bottom').length > 0) {
        ...
        $('.chat_wiew .chat_text,.chat_wiew .chatText,.chat_wiew .chat_holder').click(function(e) {
          $('.chat_wiew .chat_holder').hide();
          $('.chat_wiew .chatText').focus();
          e.stopPropagation();
        });
        $('#recv_chat_window').focus(function(e) {

        }).blur(function(e) {
          if ($.trim($(this).text()) == '') {
            $(this).val('')
            $('.chat_wiew .chat_holder').show();
          }
        });
        this.chat_box_scrollbar = $('#chat_box_scrollbar').tinyscrollbar({
          trackSize: parseInt($('#chat_box_scrollbar .viewport').attr('sh')) - 4
        });
      } else {
        this.chat_box_scrollbar = $('#chat_box_scrollbar').tinyscrollbar({
          trackSize: parseInt($('#chat_box_scrollbar .viewport').attr('sh')) - 4
    ...
    chatTab: function(e) {
      if (!$(e.target).hasClass('on')) {
        ...
        $('.chat_wiew.hide').removeClass('hide').siblings('.chat_wiew').addClass('hide');
        $('.chat_wiew:visible .scroll_up').click();
        $('.saySingle').remove();
      }
    },
    ...
    showDropDown: function() {
      ...
      $("#emoticonDropDown").css({
        ...
        top: buttonPosition.top - $("#emoticonDropDown").outerHeight() - 12
      });
      $("#emoticonDropDown").find('.emoticonsTab a').click(function() {
      ...
      $('#emotionBarsDefaults').tinyscrollbar({
        trackSize: 136
      });
      $('#emotionBarsVip').tinyscrollbar({
        trackSize: 136
      });
      $('#emotionBarsAngel').tinyscrollbar({
        trackSize: 136
      });
    },
    hideDropDown: function() {
      $('.emoticon').removeClass('hover');
    ...
    addmsg: function(euid, name, content, is) {
      ...
      try {
        ...
        var ii = $('#chat_box p').length - 3000
        if (ii > 0) {
          $('#chat_box p:lt(' + ii + ')').remove();
        }
    ...
    showAudience: function() {
      ...
      setInterval($.getJSON(apiUrl + 'online/getUserlist', {
        ...
        tdiv.html(str);
        $('#userListScrollbar').tinyscrollbar({
          trackSize: 214
        })
      }), 60000);
    },
    ...
    showBuyVip: function() {
      ...
      emoTips1.show();
      $('.pop-msg-emos .ui-dialog-close').text('');
